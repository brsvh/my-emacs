...
For certain patterns or commands that require a substantial time
expenditure, I also use a more granular level of control, which hooks
with an ~on-~ prefix.
...
Moreover, for Emacs sessions with and without Frames, that is, Emacs
sessions initiated in graphics or in character-only terminals, I have
two hooks for initialization. They will be evaluated at the end of the
~after-init-hook~.

#+begin_src emacs-lisp
  (defcustom my-frame-init-hook nil
    "Hook run for initialize Frame."
    :group 'my-hooks
    :type 'hook)

  (defcustom my-noframe-init-hook nil
    "Hook run for initialize text terminal."
    :group 'my-hooks
    :type 'hook)
#+end_src

The ~my-frame-init-hook~ is somewhat unique, designed to refine the
categorized evaluation of Emacs' behavior following the detection of a
graphical session.
...
~display-graphics-p~ values.  Therefore, I need to place Frame-specific
code in ~after-make-frame-functions~.

#+begin_src emacs-lisp
  (defun my-frame-initialization (&rest _)
    "Initialization of Emacs session with Frame."
    (run-hooks 'my-frame-init-hook))

  (defun my-noframe-initialization (&rest _)
    "Initialization of Emacs session with Frame."
    (run-hooks 'my-noframe-init-hook))

  (add-hook 'after-init-hook #'my-frame-initialization 100)

  (add-hook 'after-make-frame-functions #'my-frame-initialization 100)

  (add-hook 'tty-setup-hook #'my-noframe-initialization 100)
#+end_src

Because the ~after-make-frame-functions~ isn’t run for the initial
frame, so it’s often necessary to also add ~my-frame-initialization~ to
the ~after-init-hook~.

For Emacs session with Frame, arranged according to the startup
sequence, it can be summarized as:

- Evaluation of the ~user-init-file~ file (this file after tangle).
- Execution of the ~after-init-hook~  hook.
- Execution of the ~my-frame-init-hook~  hook.
- Execution of the ~emacs-startup-hook~  hook.

For Emacs session without Frame:

- Evaluation of the ~user-init-file~ file (this file after tangle)
- Execution of the ~after-init-hook~  hook.
- Execution of the ~my-noframe-init-hook~  hook.
- Execution of the ~emacs-startup-hook~  hook.
...
~dash~[fn:4] provides encapsulation of the List operation API.
...
For frequently used functions, I want to modify variables based on their
inherent values, akin to the ~setq~ function.
...
~f~[fn:5] provides encapsulation of the File and Path operation API.
...
~s~[fn:6] provides encapsulation of the String operation API.
...
Construct the path while simultaneously creating its parent directory.

#+begin_src emacs-lisp
  (defun my-expand-file-name-with-make-directory (path &optional dir)
    "Expand PATH relative to DIR with create its parents.
  PATH and DIR can be either a directory names or directory file
  names.  Return a directory name if PATH is a directory name, and
  a directory file name otherwise.  The parent directory of result
  will be create at the same time."
    (or dir (setq dir default-directory))
    (let ((p (f-expand path dir)))
      (f-mkdir-full-path (f-dirname p))
      p))
#+end_src

Some are used to generate path that are located in a ~my-~ directory.

#+begin_src emacs-lisp
  (defun my-cache-expand (path)
    "Expand PATH relative to `my-cache-directory'."
    (my-expand-file-name-with-make-directory path my-cache-directory))

  (defun my-config-expand (path)
    "Expand PATH relative to `my-config-directory'."
    (my-expand-file-name-with-make-directory path my-config-directory))

  (defun my-data-expand (path)
    "Expand PATH relative to `my-data-directory'."
    (my-expand-file-name-with-make-directory path my-data-directory))

  (defun my-state-expand (path)
    "Expand PATH relative to `my-state-directory'."
    (my-expand-file-name-with-make-directory path my-state-directory))
#+end_src

*** Macros

A macro is used to incorporate the content to be evaluated as a lambda
function into ~my-frame-init-hook~, when the current session is
~display-graphic-p~.

#+begin_src emacs-lisp
  (defmacro my-when-graphic-frame (&rest body)
  ...
  "
    ...
    `(when (display-graphic-p)
       (add-hook 'my-frame-init-hook
                 #'(lambda (&rest _) (progn ,@body)))))
#+end_src

** ~refomatter~

~reformatter~[fn:7] provides simple macros for define new formatter
commands and minor modes.
...
~transient~[fn:8] implements an abstraction that encompasses a prefix
...
#+begin_src emacs-lisp
  (use-package transient
    ...
    :defer t
    :config
    (setq transient-history-file (my-state-expand "transient/history.el")
          transient-levels-file (my-state-expand "transient/levels.el")
          transient-values-file (my-state-expand "transient/values.el")))
#+end_src
...
provided by gcmh[fn:9].  This strategy utilizes a low Garbage Collection
...
#+begin_src emacs-lisp
  (use-package server
    ...
    :config
    (setq server-auth-dir (my-state-expand "server/"))
...
#+begin_src emacs-lisp
  (use-package cus-edit
    :init
    (setq custom-file (my-state-expand "custom.el"))
...
I employ moody[fn:10] to acquire a mode line styled with tabs and ribbons.
...
employ ~minions~[fn:11] to supplant the minor modes list with a menu button
...
system. Subsequently, I use Modus Themes[fn:12] that enhances Emacs'
  ...
  (use-package modus-themes
      ...
      (setq modus-themes-custom-auto-reload t
            ...
            modus-themes-headings '((0 . (1.40 ultrabold))
                                    ...
                                    (t . (1.05 semibold))))
      :hook
      (my-frame-init-hook . my-theme-init)
      (my-noframe-init-hook . my-theme-init))
#+end_src
...
~switch-window~[fn:13] as a remedy.
...
My choice is to provided input completion support with ~vertico~ [fn:14],
...
input using spaces as separators.  Consequently, I use orderless[fn:15],
...
of the minibuffer for your completion candidates.[fn:16]
...
#+begin_src emacs-lisp
  (use-package savehist
    :config
    (setq savehist-file (my-state-expand "hist.el"))
    :hook
...
I use Embark[fn:17] to summon additional shortcut actions at the current
...
Emacs supports bookmarking a file for swift navigation, it is
impeccable, except for its non-compliance with my file conventions.

#+begin_src emacs-lisp
  (use-package bookmark
    :config
    (setq bookmark-default-file (my-data-expand "bookmarks.el")))
#+end_src
...
I opt for the tried-and-tested ~company-mode~[fn:19] as the primary text
...
Upon the creation of auto-saved files, Emacs will archive the paths of
all such files into a document designated by the value
of~'auto-save-list-file-name~.

#+begin_src emacs-lisp
  (use-package startup
    :no-require t
    :init
    (setq auto-save-list-file-prefix (my-data-expand "auto-save/lists/")))
#+end_src
...
#+begin_src emacs-lisp
  (use-package saveplace
    :config
    (setq save-place-file (my-state-expand "place.el"))
    :hook
...
files I've recently opened and their paths.
...
#+begin_src emacs-lisp
  (use-package recentf
    ...
    (setq recentf-save-file (my-state-expand "recent.el"))
    :hook
...
are easily readable and they work nicely with version control
systems.[fn:20]

The versatility of the Editor is exceptional, hence I have also enabled
Emacs to support it.
...
obtain this feature through ~diff-hl~[fn:21].
...
specifically, Emacs equipped with Magit[fn:22].  I am acquainted with
...
experimenting with Nix3.el[fn:23], an Emacs Nix Interface akin to Magit,
...
Emacs enforces network security through the Network Security Manager.
Currently, this is accomplished by utilizing the functionality of
Transport Layer Security (TLS).  Whenever Emacs establishes any network
connection, it passes the established connection to it.  I have
customized its settings to ensure the security of network access within
Emacs.
...
#+begin_src emacs-lisp
  (use-package nsm
    ...
    (setq network-security-level 'medium
          nsm-settings-file (my-state-expand "network-security.data")))
#+end_src
...
#+begin_src emacs-lisp
  (defvar my-authinfo-file (my-data-expand "authinfo")
    "My autoinfo file.")

  (defvar my-authinfo-gpg-file (my-data-expand "authinfo.gpg")
...
Reading has occupied a significant portion of my life, so how should I
read in Emacs? News, emails, books, and even the entire world.

** Read NEWS using ~gnus~

I use ~gnus~ to peruse the majority of information originating from
the external world, such as news disseminated via NNTP Server, as well
as emails intended for reading (in contrast to engaging in conversation
with individuals).

#+begin_src emacs-lisp
  (use-package gnus
    :init
    (setq gnus-directory (my-data-expand "news"))
    :config
    (setq gnus-init-file (my-config-expand "gnus.el")
          gnus-registry-cache-file (my-cache-expand "gnus.registry.eieio")
          gnus-select-method '(nnnil "")
          gnus-startup-file (my-state-expand "gnus.newsrc"))

    (-snocq gnus-secondary-select-methods
            '(nntp "gmane" (nntp-address "news.gmane.io"))))

  (use-package gnus-cloud
    :after gnus
    :config
    (setq gnus-cloud-synced-files `(,my-authinfo-gpg-file
                                    ,gnus-init-file
                                    (:directory ,gnus-directory
                                     :match ".*.SCORE\\'"))))
#+end_src

~gnus~ is inherently slow, I expedite it simply by enabling asynchronous
...
Furthermore, personalize the appearance of the ~gnus~ group and news.
...
#+begin_src emacs-lisp
  ...
  (use-package gnus-sum
    ...
    (setq gnus-summary-line-format (concat "%0{%U%R%z%}"
                                           "%1{%o%}"
                                           "  "
                                           "%4{%-20,20f%}"
                                           "  "
...
When I have finished reading all the news, and there are no fresh
articles on the NNTP server, gnus buffer appears blank.  This induces a
modicum of unease in me, hence I always ensure their visibility.

#+begin_src emacs-lisp
  (use-package gnus-group
    :config
    (setq gnus-permanently-visible-groups "^nntp"))
#+end_src


** Reading E-Mail using and ~mu4e~
...
concurrently use ~mu4e~ for email management.

#+begin_src emacs-lisp
  (use-package message
    :config
    (setq message-signature-file (my-config-expand "mail/signature")))

  (use-package mail-source
    :config
    (setq mail-source-crash-box (my-data-expand "mail/crash-box")))

  (use-package sendmail
    :config
    (setq mail-personal-alias-file (my-config-expand "mail/mailrc")
          mail-signature-file (my-config-expand "mail/signature")))

  (use-package rmail
    :config
    (setq rmail-file-name (my-data-expand "rmail")))
  ...
  (use-package nnmail
    :config
    (setq nnmail-message-id-cache-file (my-cache-expand "nnmail-cache")))
#+end_src

I am in the process of configuring my email account via home-manager,
and it look like the follow example.
...
#+begin_src emacs-lisp :tangle no
  (use-package smtpmail
    ...
    (setq
     ...
     smtpmail-smtp-user "foo.bar@example.org"
     smtpmail-stream-type 'ssl
     smtpmail-local-domain "localdmain"))
#+end_src

When I using ~mu4e~, its default completion interface is ~ido~, which is
not sufficiently user-friendly.  I uniformly employ ~vertico~ completing
interface to provide completing function.
...
I want to ~mu4e~ displaying more aesthetically pleasing icons when
initiate it within the GUI Emacs.
...
Emacs supports a variety of line-folding methods[fn:24], including:
...
During my writing process, I intermittently incorporate literary markers
such as =‘= and =’=、 =“= 和 =”=, =«= and =»=, etc.  I anticipate that
upon inputting the left marker, the corresponding right marker will be
automatically inserted. I achieve this through the *Electric Pair* Mode.
It will automatically insert the corresponding symbols in the pair mark
based on the current buffer's syntax table.
...
~org-modern~[fn:25] to enhance the appearance of Org Mode headings by
...
When I insert characters of multiple languages into the tables in Org,
they are predominantly visually unaligned, specifically when inserting
both Latin and CJK characters.  I achieve superior visual alignment on
GUI Emacs for Org Mode through valign[fn:27].
...
When using the automatic indentation feature of Org Mode, due to the
extensive length of the text being edited, it is hard to discern the
current indentation level at a glance.  However, by employing visible
indentation lines, one can swiftly comprehend the current level.

#+begin_src emacs-lisp
  (use-package org-visual-indent
    :ensure org-visual-outline
    :after org
    :config
    (defun my-prefer-to-use-org-visual-indent (&rest _)
      "Prefer to use `org-visual-indent-mode'."
      (when org-startup-indented
        (org-visual-indent-mode)))

    (add-hook 'org-mode-hook #'my-prefer-to-use-org-visual-indent))
#+end_src
...
extensive works.  I use ~org-sidebar-tree~[fn:26] to achieve this.
...
#+begin_src emacs-lisp
  (use-package org-side-tree
    ...
    :hook
    (org-side-tree-mode-hook . org-visual-indent-mode))
#+end_src


*** Electric pair

When I use Org Mode, I aspire to automatically insert a corresponding
markup symbol upon typing one.
...
Markdown[fn:28], perhaps the most prevalent text markup language of the first
...
In Org Mode, I can utilize ~org-edit-special~ ({{{kbd(C-c ')}}}) to edit
text located within specialized blocks in an indirect buffer in the
native major mode.  This feature is highly convenient, and I also employ
its Markdown variant.
...
Similarly, enable superior alignment support for Markdown tables.
...
I use the *Electric Indent* mode to acquire automatic indentation
support.  Electric Indent is character-triggered, defaulting to
activation only through new line.  I want that the {{{kbd(<DEL>)}}} can
similarly trigger an alteration in indentation.
...
I use smartparens[fn:29] as a substitute for the default Electric Pair
mode, employing it to maintain parentheses balance throughout the
development process in most programming languages.
...
I use Tree-sitter[fn:30] to augment my programming experience, acquiring
enhanced display, indentation, and structured editing through syntax
trees.  It is a small, fast C library that parses source code, creating
a parse tree.  And its integration of Emacs is built in after 29, so
it's handy.  I strive to use the primary modes that support Tree-sitter,
particularly when programming.

Another crucial aspect during programming is the utilization of a
formatter for automatic code formatting, contributing to the robustness,
correctness, and aesthetics of the code.  I opt for ~reformatter~,
which permits me to format the current buffer and upon saving.

** Lisp

Developing Lisp code is a joyous endeavor, yet the incessant
modification of parentheses within Lisp code is an exceedingly arduous
task.  In the past, formidable Emacs developers have crafted a series of
plugins to facilitate the development of Lisp code, such as ~lispy~,
~paredit~, and others.  While they are commendable, each possesses its
own set of drawbacks, such as complex configuration and performance
issues.  Currently, ~parinfer-rust-mode~ appears to be gaining
popularity, and I have opted to select it.
...
#+begin_src emacs-lisp
  (use-package parinfer-rust-mode
    ...
    :config
    (defvar-local my--electric-pair-mode-state nil
      "The state of `electric-pair-mode' in a buffer.")

    (defun my--parinfer-rust-mode-inhibit-electric-pair-mode (&rest _)
      "Disable `electric-pair-mode' when use `parinfer-rust-mode'."
      (setq-local my--electric-pair-mode-state electric-pair-mode)
      (if (bound-and-true-p parinfer-rust-mode)
          (electric-pair-local-mode -1)
        (when my--electric-pair-mode-state
          (electric-pair-local-mode +1))))

    (add-hook 'parinfer-rust-mode-hook
              #'my--parinfer-rust-mode-inhibit-electric-pair-mode))
...
Employ ~parinfer-rust-mode~ concurrently to maintain the parentheses
balance and indentation.
...
As you see, I employ ~nixpkgs-fmt~ as my Nix file formater.

** YAML

YAML[fn:31] is an exceedingly prevalent configuration language. Despite my
personal aversion towards it, I have to incorporate its support.

#+begin_src emacs-lisp
  (reformatter-define my-yaml-format :program "nixpkgs-fmt")
...
Emacs*[fn:32] by Nicolas P. Rougier[fn:33] provided me with substantial
...
[fn:4] dash.el, https://github.com/magnars/dash.el

[fn:5] f.el, https://github.com/rejeep/f.el

[fn:6] s.el, https://github.com/magnars/s.el

[fn:7] reformatter, https://github.com/purcell/emacs-reformatter

[fn:8] Transient, https://github.com/magit/transient

[fn:9] GCMH Mode, https://gitlab.com/koral/gcmh

[fn:10] Moody Mode Line, https://github.com/tarsius/moody

[fn:11] Minions, https://github.com/tarsius/minions

[fn:12] Modus Themes, https://protesilaos.com/emacs/modus-themes

[fn:13] Switch Window, https://github.com/dimitri/switch-window

[fn:14] Vertico, https://github.com/minad/vertico

[fn:15] Orderless, https://github.com/oantolin/orderless

[fn:16] Marginalia, https://github.com/minad/marginalia

[fn:17] Embark, https://github.com/oantolin/embark

[fn:18] which-key, https://github.com/justbur/emacs-which-key

[fn:19] Company Anything, http://company-mode.github.io/

[fn:20] EditorConfig, https://editorconfig.org/

[fn:21] diff-hl, https://github.com/dgutov/diff-hl

[fn:22] Magit, https://magit.vc

[fn:23] Nix3.el, https://github.com/emacs-twist/nix3.el

[fn:24] Line Wrap, https://www.emacswiki.org/emacs/LineWrap

[fn:25] Modern Org Style, https://github.com/minad/org-modern

[fn:26] Org Side Tree, https://github.com/localauthor/org-side-tree

[fn:27] Valign.el, https://github.com/casouri/valign

[fn:28] Markdown, https://daringfireball.net/projects/markdown/

[fn:29] smartparens, https://github.com/Fuco1/smartparens

[fn:30] tree-sitter, https://tree-sitter.github.io/tree-sitter/

[fn:31] YAML, https://yaml.org/

[fn:32] Get Things Done with Emacs, https://www.labri.fr/perso/nrougier/GTD/index.html

[fn:33] Nicolas P. Rougier, https://www.labri.fr/perso/nrougier/
