...
#+begin_src emacs-lisp
  (use-package gnus
    ...
    (-snocq gnus-secondary-select-methods '(nntp "news.gmane.io")))
...
~gnus~ is inherently slow (in 2024), I expedite it simply by enabling
...
** Reading E-Mail using ~gnus~ and ~mu4e~
...
#+begin_src emacs-lisp :export none :tangle no
  ...
  (use-package gnus
    :config
    (push '(nnimap "example.org"
                   (nnimap-address "imap.example.org")
                   (nnimap-server-port 993)
                   (nnimap-user "foo.bar@example.org")
                   (nnimap-stream 'ssl))
